...
class Haze:
    ...
    def get_loaders(self, parse_patches=True, validation='haze'):
        print("=> evaluating outdoor Haze test set...")
        train_dataset = HazeDataset(dir=os.path.join(self.config.data.data_dir, 'data', 'ntire2023/dehazing', 'train'),
        ...
        val_dataset = HazeDataset(dir=os.path.join(self.config.data.data_dir, 'data', 'ntire2023/dehazing', 'val'),
                                      n=self.config.training.patch_n,
                                      patch_size=self.config.data.image_size,
                                      transforms=self.transforms,
                                      filelist=None,
...
class HazeDataset(torch.utils.data.Dataset):
    def __init__(self, dir, patch_size, n, transforms, filelist=None, parse_patches=True, reverse_img=False):
        ...
        if filelist is None:
            ...
            # Haze train filelist
            haze_inputs = os.path.join(haze_dir, 'input')
            images = [f for f in listdir(haze_inputs) if isfile(
                os.path.join(haze_inputs, f))]
            # assert len(images) == 50000
            input_names += [os.path.join(haze_inputs, i) for i in images]
            gt_names += [os.path.join(os.path.join(haze_dir, 'gt'), i)
                         for i in images]
